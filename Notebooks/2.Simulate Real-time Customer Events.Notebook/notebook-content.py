...
# ## Pre-requisites
# - Add  lakehouse to this notebook following same steps from notebook 1.
# - Retrieve the **connection string** from the Eventstream for the Setup section
#     - In your workspace, Create or use the eventstream in the workspace
#     - Go to Details -> SAS Key Authentication and copy the `Connection string-primary key` (note down the `Endpoint`, `SharedAccessKeyName`, `SharedAccessKey` and `EntityPath` inside this connection string).
...
#
# Event-time behavior:
# - ORDERED: current timestamp - 5 days
# - PAID: ORDERED + 60 seconds
# - DELIVERED: PAID + random 1 to 5 days
# - LOST: PAID + random 1 to 5 days
...
events = (
    ...
    .withColumn(
        "orderedEpoch",
        F.expr("cast(current_timestamp() - INTERVAL 5 DAYS as long)")
    )
    .withColumn("paidEpoch", F.col("orderedEpoch") + F.lit(60))
    .withColumn(
        "postPaidLagDays",
        F.when(
            F.col("status").isin("DELIVERED", "LOST"),
            (F.floor(F.rand() * F.lit(5)) + F.lit(1)).cast("long")
        ).otherwise(F.lit(0))
    )
    .withColumn(
        "eventTime",
        F.when(F.col("status") == "ORDERED", F.col("orderedEpoch").cast("timestamp"))
         .when(F.col("status") == "PAID", F.col("paidEpoch").cast("timestamp"))
         .otherwise(
             (F.col("paidEpoch") + F.col("postPaidLagDays") * F.lit(86400)).cast("timestamp")
         )
    )
    .drop("timestamp", "statusStep", "orderedEpoch", "paidEpoch", "postPaidLagDays")
)
